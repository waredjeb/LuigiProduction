...
def writeHTCondorDiscriminatorFiles_outputs(args):
    ...
    check_name = 'Discriminator_C$(Cluster)P$(Process).o'
    ...
    return jobFile, submFile, checkFile

@utils.setPureInputNamespace
def writeHTCondorDiscriminatorFiles(args):
    script = os.path.join(args.localdir, 'scripts')
    script = os.path.join(script, 'variableImportanceDiscriminator.py')
    prog = 'python3 {}'.format(script)

    out_job, out_submit, out_check = writeHTCondorDiscriminatorFiles_outputs(args)
    ...
    queue = 'short'
    with open(out_submit, 'w') as s:
        s.write('Universe = vanilla\n')
        s.write('Executable = {}\n'.format(out_job))
        s.write('Arguments = $(channel) \n')
        s.write('input = /dev/null\n')
        s.write('output = {}\n'.format(out_check))
        s.write('error  = {}\n'.format(out_check.replace('.o', '.e')))
        s.write('getenv = true\n')
        s.write('T3Queue = {}\n'.format(queue))
        s.write('WNTag=el7\n')
